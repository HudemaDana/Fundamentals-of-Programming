...
class RepoActivity:
    ...
    def add_activity(self, activity):
        ...
        :return: verifies if any unique data repeats and if so, it generates a new list of people until it finds it as
                 unique
        """
        for i in range(0, len(self._act_data)):
            if activity.date == self._act_data[i][1] and activity.time == self._act_data[i][2]:
                for j in range(0, len(self._act_data[i][4])):
                    while self._act_data[i][4][j] in activity.person_id:
    ...
    def repo_activity(self):
        return self._act_data

    def __getitem__(self, item):
        return self._act_data[item]

    @repo_activity.setter
    def repo_activity(self, new_list):
        self._act_data = new_list
    ...
    def random_generate_activity(self):
        ...
        for i in range(20):
            ...
            for j in range(len(self._act_data)):
                if self._act_data[j][1] == date and self._act_data[j][2] == time and self._act_data[j][
                    3] == description:
                    description = random.choice(self.generate_description())
